# --- HOTFIX: replace / paste this whole class into src/action_tracking/data/repositories.py ---
# It restores missing ProductionDataRepository methods used by production_import.py and other pages.
...
# IMPORTANT:
# - If normalize_wc is in a different module in your repo, adjust the import below.
# - This import is used only by list_production_work_centers_with_stats().
...
class ProductionDataRepository:
    ...
    def upsert_scrap_daily(self, rows: list[dict[str, Any]]) -> None:
        """
        Upsert aggregated daily scrap rows.
        ...
        Expected row keys:
          ...
          - scrap_cost_currency (string, default 'PLN')
          - id (optional)
          - created_at (optional)
        """
        if not rows:
    ...
    def upsert_production_kpi_daily(self, rows: list[dict[str, Any]]) -> None:
        """
        Upsert aggregated daily KPI rows.
    ...
    # Backward-compatible alias (some pages might still call this)
    ...
    def list_production_work_centers_with_stats(self) -> list[dict[str, Any]]:
        """
        Used by WC inbox feature: returns per-normalized-WC stats merged from scrap + KPI.
        """
